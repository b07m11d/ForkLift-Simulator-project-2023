...
public class ScoreGroupComp : MonoBehaviour
{
    ...
    string ForkPositionHightScoreTxt = "堆高機行駛前未事先升高貨叉15-20cm";
    string ForkMastTiltScoreTxt = "堆高機行駛前未事先傾斜貨叉3-5cm";
    ...
    string ForkInsertGoodsTxt = "貨叉插入棧板";
    string GoodsOnShelfTxt = "貨物已經放置在架上";
    bool isForkliftAlmostTouch = false;
    bool isForkInsertGoods = false;
    bool isGoodsOnShelfTxt = false;
    ...
    void Start()
    {
        ...
        switch (SceneManager.GetActiveScene().name)
        {
            ...
            case "ThridStage":
                ...
                //========第三關====================2023-11-08 by lin liang yu 等號右邊的，是下面自己寫的function 名稱
                scoreManager.OnGoodsOnShelfOK += GoodsOnShelfOK;
                scoreManager.OnForkInsertGoods += ForkInsertGoods;
                scoreManager.OnAlmostTouch += AlmostTouch;
...
/*                int GoodsOnShelfOK_C;
                ...
                //貨叉插入貨物
                if (IfGoodsOnMe.isGoodsOnMe_fk == true && isForkInsertGoods == false && IfGoodsOnMe.ThisisGoodsShelf == false)
                {
                    //ForkInsertGoods(ForkInsertGoods_C);
                    isForkInsertGoods = true;
                    //return;
                }
                else if (IfGoodsOnMe.isGoodsOnMe_fk == false && IfGoodsOnMe.ThisisGoodsShelf == false)
                {
                ...
                //貨物放上貨架
                if (IfGoodsOnShelf.isGoodsOnMe_sf == true && isGoodsOnShelfTxt == false && IfGoodsOnShelf.ThisisGoodsShelf == true)
                {
                    //GoodsOnShelfOK(GoodsOnShelfOK_C);
                    isGoodsOnShelfTxt = true;
                    //return;
                }
                else if (IfGoodsOnShelf.isGoodsOnMe_sf == false && IfGoodsOnShelf.ThisisGoodsShelf == true)
                {
                    //isCheckOnRoadLine = false;
                    isGoodsOnShelfTxt = false;
                }
    ...
    void AlmostTouch(int i) {
        //練習模式
        PracticeModeUI(AlmostTouchTxt, i);

        //測驗模式
        TestModeUI(AlmostTouchTxt, i);
    }

    void ForkInsertGoods(int i) {
        //練習模式
        PracticeModeUI(ForkInsertGoodsTxt, i);

        //測驗模式
        TestModeUI(ForkInsertGoodsTxt, i);
    }

    void GoodsOnShelfOK(int i) {
        //練習模式
        PracticeModeUI(GoodsOnShelfTxt,i);

        //測驗模式
        TestModeUI(GoodsOnShelfTxt, i);
    }
